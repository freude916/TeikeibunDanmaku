...
## JSONC 文件分层约定（2026-04）

`rules/` 目录内统一按后缀分流：

- `*.danmu.jsonc`：规则定义文件，仅包含规则。
- `*.cards.jsonc`：卡牌流派/终端定义文件，仅包含卡牌元数据。

两类文件都采用对象根，必须带版本号，便于编辑器后续扩展：
...
- `RuleJsoncIo` 只负责 `.danmu.jsonc`。
- `CardArchetypeJsoncIo` 只负责 `.cards.jsonc`。
- 不跨层，不在规则 loader 里处理卡牌定义。
