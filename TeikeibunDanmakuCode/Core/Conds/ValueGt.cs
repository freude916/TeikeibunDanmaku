...
public sealed class ValueGtListCount(BoardFieldDescriptor fieldDescriptor, string item, double expectedCount)
    : ICondition
{
    private readonly BoardFieldDescriptor _fieldDescriptor = fieldDescriptor ?? throw new ArgumentNullException(nameof(fieldDescriptor));
...
public sealed class GtConditionCodec : ConditionCodec
{
    ...
    public override ICondition DeserializeDto(ConditionDto dto, Type stateType, ConditionDeserializer deserializer)
    {
        ...
        {
            throw new JsonException($"Condition '{ConditionType.ValueGt}' requires numeric key or string-list key '{key}'.");
        }

        var (item, count) = ParseListCountValue(value, key);
        return new ValueGtListCount(descriptor, item, count);
    }
